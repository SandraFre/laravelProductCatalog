    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                ...
                <form
                    action="{{ route('products.' . (isset($product->id) ? 'update' : 'store'), isset($product->id) ? ['product' => $product->id] : []) }}"
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            name="type" value="{{$key}}">
                            <label class="form-check-label" for="type_{{$key}}">{{$enum->name()}}</label>
                        ...
                        <div class="form-group">
                            <label for="suppliers">Suppliers:</label> <br>
                            @foreach($suppliers as $id => $title)
                            <input type="checkbox" id="suppliers" name="suppliers[]" value="{{ $id }}" @if(in_array($id,
                                old('suppliers', $supplierIds ?? []))) checked @endif> {{ $title }}
                            @endforeach
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="@error('active') is-invalid @enderror" type="checkbox" name="active"
                                id="active" value="1" @if (old('active', $product->active ?? false)) checked @endif>
                            @error('active')
                            <div class="alert-danger">
                                {{ $message }}
                            </div>
